...
public class RedLong extends LinearOpMode{
    ...
    public void runOpMode() throws InterruptedException{
        ...
        Thread.sleep(1100);             // increase if doesn't turn right enough, decrease if turn too far right
        forward(drive_power);
        Thread.sleep(3200);             // increase if doesn't go forward enough second time, decrease if too far forward
        strafeLeft(drive_power);
        Thread.sleep(1500);             // increase if doesn't go left enough, decrease if go too far left
        stopDriving();
        servosUp.start();
